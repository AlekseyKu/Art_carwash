# Фича: UI PWA — порт дизайн-системы Cyberdom mobile

**Статус:** shipped  
**Связано:** [customer-app.md](customer-app.md), [brand.md](../product/brand.md), `apps/pwa`  
...
| Тема | Решение |
|------|---------|
| Бренд | «Гранат» + графит, **светлая** тема — [brand.md](../product/brand.md) |
...
- [x] `theme/tokens.css` — полный набор из [brand.md](../product/brand.md) + spacing/radii из Cyberdom
...
- [x] `ClassTabs` — горизонтальный скролл с иконками классов
- [x] `ListRow` — название + цена, разделители как в `EventListTile`
- [x] Карточка «О мойке» — CTA «Позвонить» / «Маршрут»
- [x] Пустое состояние каталога

### UI-4 — Онбординг и сплэш (0.5–1 день)

- [x] Сплэш: графит + лого «Автомойка у ЖД» (без frame Cyberdom или свой паттерн)
- [x] Welcome: 3 тезиса в `Card`, крупная типографика
- [x] Анимация появления 400ms `ease-in-out` (как Cyberdom)

### UI-5 — Кабинет и заглушки (0.5 дня)

- [x] Профиль, logout, stub гаража в едином стиле
- [x] `/owner` — минимальный dark/admin stub

### UI-6 — PWA polish (0.5 дня)

- [x] Иконки `pwa-192/512` — бренд «у ЖД», не Cyberdom logo
- [x] `navigateFallbackDenylist` для `/api/*` (уже в коде)
- [x] iOS install hint (опционально, по образцу `PwaInstallBanner`)
...
- [x] Нижняя навигация с акцентом активного таба (+ лого → home)
- [x] Прайс: класс авто → вкладки → список с ценами
- [x] Палитра «Гранат» + графит ([brand.md](../product/brand.md)), не Cyberdom purple
- [x] Touch targets / кнопки записи без системной синей подсветки
- [ ] Lighthouse PWA + a11y — прогнать при следующей полировке
- [ ] Визуальный паритет «как нативное» на iPhone/Android — субъективно, добить по фидбеку
...
| Раздувание CSS | `theme/` + компоненты, без UI-библиотеки |
| Flutter ≠ React parity | UX parity, не pixel-perfect |

---

## Статус

**shipped.** Дальше — точечная полировка по фидбеку, не полный порт Cyberdom.
